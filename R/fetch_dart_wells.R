# fetch_dart_wells.R
# -------------------------------------------------------------
# Weekly pull of Wells Dam adult Pacific Lamprey passage counts
# from CBR DART. Writes data/wells_lamprey_counts.csv with one
# row per date covering every year from 2010 through the current
# year.
#
...
# The response includes counts for all species at Wells, keyed by
# columns like Chin/Stlhd/BTrout/etc; the lamprey columns are
# LmpryDay (daily count) and LmpryDay10Yr (10-yr average).
#
...
dir_create(data_dir)
out_fp   <- path(data_dir, "wells_lamprey_counts.csv")

# ---- DART URL template --------------------------------------
# Verified working for years where Wells has lamprey data.
...
pull_year <- function(yr) {
  ...
  resp <- request(url) |>
    req_user_agent("lamprey-dashboard (R httr2)") |>
  ...
  df <- df |>
    select(any_of(c("Project", "Date", "LmpryDay", "LmpryDay10Yr"))) |>
    rename_with(tolower)

  if (!"lmpryday" %in% names(df)) {
    message(glue("  {yr}: no LmpryDay column in response, skipping"))
    return(NULL)
  }
  if (!"lmpryday10yr" %in% names(df)) df$lmpryday10yr <- NA_real_

  df |>
    mutate(
      date         = suppressWarnings(as.Date(date)),
      lmpryday     = suppressWarnings(as.numeric(lmpryday)),
      lmpryday10yr = suppressWarnings(as.numeric(lmpryday10yr))
    ) |>
    filter(!is.na(date)) |>
    transmute(
      date,
      count          = ifelse(is.na(lmpryday), 0L, as.integer(lmpryday)),
      count_10yr_avg = lmpryday10yr
    )
}
...
if (is.null(all) || nrow(all) == 0) {
  ...
  all <- tibble::tibble(date = as.Date(character()),
                         count = integer(),
